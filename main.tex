...
Soit un échantillon univarié ordonné : $y_{(1)} \le y_{(2)} \le \dots \le y_{(n)}$.

\subsection{Tendance centrale}
\textbullet{} \textbf{Moyenne empirique} : $\overline{y} = \frac{1}{n}\sum_{i=1}^{n}y_{i}$. Très sensible aux valeurs aberrantes.\\
\textbullet{} \textbf{Médiane ($med(y)$)} : Indicateur robuste (résistant).
...
\textbullet{} \textbf{Variance empirique ($s^2$)} : Division par $(n-1)$ pour corriger le biais. Formule de König-Huygens empirique associée :
...
\subsection{Propriétés des Espérances conditionnelles}
\textbullet{} \textbf{Théorème de l'espérance totale} : Soit une partition de l'espace fondamental $A_1, A_2, \dots$
...
\textit{Propriétés de la covariance} :
\begin{enumerate}
    \item $cov(X,Y) = cov(Y,X)$ et $cov(X,X) = var(X)$.
    \item Bilinéarité : $cov(X+Y, Z+W) = cov(X,Z) + cov(Y,Z) + cov(X,W) + cov(Y,W)$.
    \item Changement d'échelle : $cov(aX+b, cY+d) = ac \cdot cov(X,Y)$.
    \item Variance d'une somme/différence : $var(X \pm Y) = var(X) + var(Y) \pm 2cov(X,Y)$.
    \item Si $X, Y$ indépendantes $\implies cov(X,Y) = 0$ (la réciproque est fausse).
\end{enumerate}
...
\textit{Propriétés de la corrélation} :
\begin{enumerate}
    \item $\rho(a+bX, c+dY) = \text{sign}(bd)\rho(X,Y)$.
    \item $\rho(X,X) = 1$ et $\rho(X,-X) = -1$ (si $X$ non constante).
    \item $\rho_{X,Y} = 0$ n'implique pas l'indépendance (relation non linéaire possible).
\end{enumerate}
...
Vraisemblance $L(\theta) = \prod_{i=1}^{n}f(x_{i};\theta)$, et log-vraisemblance $l(\theta) = \sum_{i=1}^{n}\log f(x_{i};\theta)$.\\
...
\textbullet{} \textbf{Erreur Quadratique Moyenne (EQM)} :
$$EQM_\theta(\hat{\theta}) = \mathbb{E}_\theta\{(\hat{\theta}-\theta)^{2}\} = Var_\theta(\hat{\theta}) + [b_\theta(\hat{\theta})]^2$$
...
\textbullet{} \textbf{Information de Fisher (un échantillon)} :
...
\subsection{Lien avec le Maximum de Vraisemblance}
La log-vraisemblance globale du modèle gaussien s'écrit :
$$l(a,\beta,\sigma^2)=$$ 
$$ -\frac{n}{2}\log\sigma^2 - \frac{1}{2\sigma^2}\sum_{j=1}^n \{y_j-(a+\beta x_j)\}^2 - \frac{n}{2}\log(2\pi)$$
Maximiser $l$ donne les mêmes estimateurs $\hat{a}_{ML} = \hat{a}_{MCO}$ et $\hat{\beta}_{ML} = \hat{\beta}_{MCO}$. Cependant, l'estimateur ML de la variance est biaisé : $\hat{\sigma}^2_{ML} = \frac{1}{n}\sum r_j^2 = \frac{n-2}{n}S_n^2$.
...
\textbf{Markov} : Soit $X$ une variable aléatoire positive ($X \ge 0$) et $a > 0$ :
